...
func Test() {
	...
	// Example: Get live volume for an event (if we have one)
	if len(events) > 0 {
		// Convert event ID from string to int (assuming it's numeric)
		eventIDStr := events[0].ID
		if eventID, err := strconv.Atoi(eventIDStr); err == nil && eventID > 0 {
			log.Printf("\nFetching live volume for event: %s", events[0].Title)
			volume, err := client.GetLiveVolume(eventID)
			if err != nil {
				log.Printf("Error fetching live volume: %v", err)
			} else {
				log.Printf("Total volume: %.2f", volume.Total)
				log.Printf("Markets with volume: %d", len(volume.Markets))
				for i, market := range volume.Markets {
					if i < 3 { // Show only first 3 markets
						log.Printf("  Market %s: %.2f", market.Market[:16]+"...", market.Value)
					}
				}
			}
		}
